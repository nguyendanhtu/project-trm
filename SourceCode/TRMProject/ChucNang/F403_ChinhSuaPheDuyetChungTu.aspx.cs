...
public partial class ChucNang_F403_ChinhSuaPheDuyetChungTu : System.Web.UI.Page
{
    ...
    private void load_data_2_cbo_dot_thanh_toan()
    {
        ...
        US_V_DM_DOT_THANH_TOAN v_us_dot_thanh_toan = new US_V_DM_DOT_THANH_TOAN();
        v_us_dot_thanh_toan.FillDataset(v_ds_dot_thanh_toan, " WHERE ID_TRANG_THAI_DOT_TT=" + get_id_trang_thai_da_duoc_duyet());
    ...
    private decimal get_id_trang_thai_da_duoc_duyet()
    {
    ...
    protected void m_cmd_cap_nhat_du_toan_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            m_cbo_dot_thanh_toan.Enabled = true;
            load_data_2_grid(get_ma_dot_tt_by_id_dot(CIPConvert.ToDecimal(m_cbo_dot_thanh_toan.SelectedValue)));
            if (m_us_v_gd_thanh_toan.dcID_TRANG_THAI_THANH_TOAN == CIPConvert.ToDecimal(hdf_id_trang_thai_thanh_toan_cu.Value))
            {
                m_lbl_thong_bao.Text = "Trạng thái của chứng từ chưa thay đổi";
            }
            else m_lbl_thong_bao.Text = "Chứng từ đã chỉnh sửa thành công";
        }
        catch (Exception v_e)
        {
            CSystemLog_301.ExceptionHandle(this, v_e);
